...
function openLocalSession(router: ReturnType<typeof useRouter>, id?: string) {
  const sessionId =
    id?.trim() || `local-${globalThis.crypto?.randomUUID?.() ?? Date.now()}`;
  router.push(`/session/${sessionId}?token=local-demo`);
}
...
export default function HomePage() {
  ...
  const joinSession = async () => {
    ...
    try {
      ...
      openLocalSession(router, joinId);
    } finally {
      setLoading(false);
    }
  };
